...
export default defineConfig({
  plugins: [
    vue(),
    cep(config),
    // publicに入れたpythonのファイルが変更されたときにdistにコピーする
    {
      name: 'copy-public-dev',
      enforce: 'post',
      handleHotUpdate({ file, server }) {
        if (file.startsWith(`${__dirname}/src/js/public`)) {
          fse.copy(`${__dirname}/src/js/public`, `${__dirname}/${devDist}/${cepDist}`);
        }
      },
    },
  ],
