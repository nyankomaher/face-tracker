...
export const isImage = (item: Item) => {
  return !!(item instanceof FootageItem && (item as any)['width']);
}

export const isMovie = (item: Item) => {
  if (!isImage(item)) return false;
  const footage = item as FootageItem;
  return !!(footage.mainSource && !footage.mainSource.isStill);
}